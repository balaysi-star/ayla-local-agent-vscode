...
function withTimeout(signal: AbortSignal | undefined, timeoutMs: number): AbortSignal {
  ...
  signal?.addEventListener("abort", () => controller.abort(new Error("CANCELLED")), { once: true });
  controller.signal.addEventListener("abort", () => clearTimeout(timeout), { once: true });
  return controller.signal;
}

async function runOnce(baseUrl: string, model: string, messages: GatewayChatMessage[], signal?: AbortSignal): Promise<StreamNormalizeResult> {
  const diagnostics = {
...
export async function normalizeOllamaStream(
  ...
  try {
    return await runOnce(config.ollamaBaseUrl, model, messages, signal);
  } catch (error) {
    const message = error instanceof Error ? error.message : "STREAM_FAILED";
    if (message === "STREAM_PARSE_FAILED" || message === "CANCELLED") {
      throw error;
    }
    const retry = await runOnce(config.ollamaBaseUrl, model, messages, signal);
