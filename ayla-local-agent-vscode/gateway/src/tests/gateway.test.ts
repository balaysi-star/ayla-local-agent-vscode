...
test("workspace validation subprocess is killed when the agent request is cancelled", async () => {
  const workspaceRoot = join(process.cwd(), ".tmp-agent-cancel-validation");
  await rm(workspaceRoot, { recursive: true, force: true });
  await mkdir(workspaceRoot, { recursive: true });
  await writeFile(join(workspaceRoot, "package.json"), JSON.stringify({
    name: "cancel-validation-fixture",
    version: "1.0.0",
    scripts: { test: "node -e \"setTimeout(() => {}, 60000)\"" }
  }), "utf8");
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(new Error("CANCELLED_BY_TEST")), 250);
  const started = Date.now();
  try {
    const result = await executeGatewayWorkspaceTool(
      { action: "run_validation", command: "npm test" },
      { allowed: true, reason: "SAFE_VALIDATION" },
      { workspaceRoot, validationTimeoutMs: 60000, signal: controller.signal }
    );
    assert.ok(Date.now() - started < 5000, "cancelled validation must return promptly");
