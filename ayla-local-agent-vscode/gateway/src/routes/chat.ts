...
import { GatewayAgentEvent, runGatewayAgentToolLoop } from "../agent/toolLoop";
...
export async function handleChatRoute(
  ...
  payload: ChatRoutePayload,
  options: { signal?: AbortSignal; onEvent?: (event: GatewayAgentEvent) => void | Promise<void> } = {}
  ...
  const result = await client.chat(model, effectiveMessages, options.signal);
