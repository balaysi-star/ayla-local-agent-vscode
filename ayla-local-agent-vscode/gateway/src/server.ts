  ...
  return createServer(async (req, res) => {
    try {
      ...
      if (method === "POST" && url.pathname === "/v1/chat/stream") {
        ...
        res.on("close", () => {
          if (!res.writableEnded && !controller.signal.aborted) controller.abort(new Error("CLIENT_DISCONNECTED"));
        });
        try {
          const result = await handleChatRoute(config, client, payload, {
            signal: controller.signal,
            onEvent: async (event) => write({ protocol: "AYLA_AGENT_EVENT_STREAM_V1", type: "event", event })
          });
          write({ protocol: "AYLA_AGENT_EVENT_STREAM_V1", type: "result", payload: result });
        } catch (error) {
          write({
            protocol: "AYLA_AGENT_EVENT_STREAM_V1",
            type: "error",
